...
5. **[medium] Inbox left rail: show extras/components as INDENTED children.** Briefed components render flat today (via the `sentOnly` union). Indent them under their parent line so the buildup reads as a hierarchy. Data's there (`project_items.parent_id`, LIVE) — expose `parentId` on `toThreadItem`, sort children after their parent, indent. **Open decision:** do indented children COUNT toward the thread total (they're separate briefed lines today) or are they display-only breakdown inside the parent's price — pin this so nothing double-counts.
