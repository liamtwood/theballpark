...
- **Schema** (`migrate-schemas.js`, all 3 schemas, idempotent): guarded `RENAME max_price → install_cost`; `DROP min_price`; `ADD currency VARCHAR(10)`, `install_description TEXT`, `location_coverage TEXT`. **Applied** to the live DB (public/preview/master) 2026-06-24, authorized by Liam. Verified: create defaults currency to the org currency (GBP), install_cost/services/coverage persist.
