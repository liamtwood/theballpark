...
5. **[medium] Inbox left rail: show extras/options as INDENTED children; hide estimate children.** **Display rule (Liam):** top-level lines (`parent_id IS NULL`) always show; **non-estimate children (`kind <> 'estimate'`) show indented** under their parent (extras/options); **`kind='estimate'` children are HIDDEN** from the rail — they're the supplier's private cost buildup (RP-11 boundary), visible only in Customize. Impl: expose `parentId`+`kind` on `toThreadItem`, filter the inbox union so a child shows only when `kind <> 'estimate'`, sort children after their parent, indent. **Note:** this refines the v2.243 `sentOnly` union, which currently shows estimate children flat (e.g. Woodworker/Planning/Set-Up Crew under Main Stage would drop out of the rail — correct, they're Main Stage's cost decomposition). **Open decision:** do indented extras/options COUNT toward the thread total (separate briefed lines today) or are they display-only breakdown of the parent's price — pin so nothing double-counts.
