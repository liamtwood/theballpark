...
const migrate = async () => {
  ...
  try {
    ...
    // ─────────────────────────────────────────────────────────────────
    // v2.09a (pV2-04) — org_type_config: per-org_type page-settings
    ...
    // localStorage). This block makes migrate-schemas.js the single
    // source of truth per The Test. Config registry: rows never go away
    // → no deleted_at (same exemption as bp_brand_config). v2 home
    // config nests under payload.v2Home so v1's flat payload fields are
    // never clobbered by v2 writes (and vice versa).
    // ─────────────────────────────────────────────────────────────────
    for (const schema of ['public', 'preview', 'master']) {
      await client.query(`
      ...
      await client.query(`
        INSERT INTO ${schema}.org_type_config (org_type, payload) VALUES
          ('agency', '{}'::jsonb), ('supplier', '{}'::jsonb), ('admin', '{}'::jsonb)
        ON CONFLICT (org_type) DO NOTHING;
      `);
    }
    console.log('  org_type_config installed + seeded (v2.09a — p0021 migration finally applied, all schemas).');
