...
3. subsubcategory: a 3rd level UNDER the chosen subcategory — ONLY when the GROUP NAME ITSELF names that specific type (e.g. supplier "Chiavari Chair Hire" → Furniture & Fixtures ▸ Seating ▸ "Chiavari Chairs"). A BROAD or GENERIC group MUST stay at the subcategory with subsubcategory "" — do NOT attach a narrow existing 3rd level to it: "Chairs"/"Chair Hire" → Seating with NO 3rd level (never "Chiavari Chairs"); "Event Furniture"/"Furniture"/"Tables" → the subcategory with NO 3rd level (never "Trestle Tables"). Only when the group is genuinely that specific: pick the best EXISTING sub-subcategory (exact string) if one truly fits, else propose a NEW one in the same house style and set subsubIsNew true. Leave subsubcategory "" whenever the group is broad, OR when the subcategory itself is NEW (isNew) — a 3rd level only nests under an EXISTING subcategory. The SAMPLE product is a SINGLE example of the group, NOT the group itself — judge 3rd-level specificity from the GROUP LABEL only, never from the sample: a broad label with a specific-looking sample (label "Chairs", sample "Folding Chair Hire") STILL stays at the subcategory with subsubcategory "".
