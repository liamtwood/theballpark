...
@Component({
  ...
  standalone: true,
  imports: [
    CommonModule, FormsModule, RouterModule, InputTextModule,
    LucideAngularModule,
    ...
    <ng-container *ngIf="!loading">
      <div class="bp-sup-search-bar">
        <div class="bp-sup-search-wrap">
          ...
          <input pInputText [(ngModel)]="searchTerm" (ngModelChange)="applyFilters()" placeholder="Search suppliers..." class="bp-sup-search-input"/>
        </div>
      </div>
      <div class="bp-sup-cats">
        <button class="bp-sup-cat-tab" [class.active]="activeCategory === 'all'" (click)="setCategory('all')">All</button>
        <button *ngFor="let cat of categories" class="bp-sup-cat-tab" [class.active]="activeCategory === cat.id" (click)="setCategory(cat.id)">{{ cat.name }}</button>
      </div>
      ...
      <div class="bp-sup-list">
        <div *ngFor="let s of filtered" class="bp-sup-card-wrap">
          <div class="bp-sup-card" (click)="toggleSupplier(s)">
            <div class="bp-sup-card-img" [style.background-image]="s.cover_image_url ? 'url(' + s.cover_image_url + ')' : null" [class.bp-sup-card-img-default]="!s.cover_image_url"></div>
          ...
          <div *ngIf="s.expanded" class="bp-sup-expanded">
            <div *ngIf="!s.catalogueLoaded" class="bp-sup-loading"><i class="pi pi-spin pi-spinner" style="font-size:13px;color:var(--theme-accent);"></i> Loading...</div>
            <div *ngIf="s.catalogueLoaded && filteredItems(s).length === 0" class="bp-sup-no-items">No items in this category.</div>
              ...
              <div class="bp-sup-tag-groups">
                <ng-container *ngFor="let group of groupedItems(s)">
                  ...
                  <div class="bp-sup-tags">
                    <button *ngFor="let item of group.items" class="bp-sup-tag" (click)="goToSupplier(s, item)">{{ item.name }}</button>
                  </div>
                </ng-container>
              </div>
              <button class="bp-sup-view-all" (click)="goToSupplier(s, null)">View all {{ filteredItems(s).length }} items &#x2192;</button>
            </ng-container>
          </div>
        </div>
      </div>
  ...
  styles: [`
    ...
    .bp-sup-cats { display:flex;overflow-x:auto;border-bottom:0.5px solid var(--color-border);background:#fff;scrollbar-width:none;padding:0 8px; }
    .bp-sup-cats::-webkit-scrollbar { display:none; }
    .bp-sup-cat-tab { padding:10px 12px;font-size:13px;font-weight:500;color:var(--color-text-muted);white-space:nowrap;background:none;border:none;border-bottom:2px solid transparent;cursor:pointer;font-family:var(--font-body);flex-shrink:0; }
    ...
    .bp-sup-card { display:flex;align-items:center;gap:12px;padding:12px 16px;background:#fff;cursor:pointer; }
...
export class SuppliersComponent implements OnInit {
  ...
  applyFilters() {
    ...
    if (this.searchTerm.trim()) { const t=this.searchTerm.toLowerCase(); result=result.filter(s=>s.name.toLowerCase().includes(t)||((s as any).city||'').toLowerCase().includes(t)); }
    if (this.activeCategory !== 'all') result=result.filter(s=>!s.catalogueLoaded||s.catalogueItems.some((i:any)=>i.category_id===this.activeCategory));
    this.filtered = result; this.cdr.detectChanges();
  }
  toggleSupplier(s: SupplierWithState) {
    s.expanded = !s.expanded;
    if (s.expanded && !s.catalogueLoaded) {
      this.supplierSvc.getCatalogue(s.id).subscribe({ next:(items:any[])=>{ s.catalogueItems=items||[]; s.catalogueLoaded=true; this.applyFilters(); this.cdr.detectChanges(); }, error:()=>{ s.catalogueItems=[]; s.catalogueLoaded=true; this.cdr.detectChanges(); } });
    }
    this.cdr.detectChanges();
  }
  filteredItems(s: SupplierWithState): any[] { return this.activeCategory==='all' ? s.catalogueItems : s.catalogueItems.filter((i:any)=>i.category_id===this.activeCategory); }
  groupedItems(s: SupplierWithState): {categoryName:string;items:any[]}[] {
    const map: Record<string,any[]>={};
    for (const item of this.filteredItems(s)) { const k=item.category_name||'Other'; if(!map[k])map[k]=[]; map[k].push(item); }
    return Object.entries(map).map(([categoryName,items])=>({categoryName,items}));
  }
  isFav(id: string): boolean { return this.favSvc.isSupplierFavourited(id); }
  toggleFav(event: MouseEvent, id: string) { event.stopPropagation(); this.favSvc.toggleSupplier(id).subscribe(()=>this.cdr.detectChanges()); }
