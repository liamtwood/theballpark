...
@Component({
      ...
      <div class="bp-body">
          ...
          <div class="bp-project-grid">
            <div *ngFor="let p of activeProjects" class="bp-project-card-wrap">
              <p-card styleClass="bp-project-card" [routerLink]="['/projects', p.id]">
                <ng-template pTemplate="header">
                  <div class="bp-card-header"
                    ...
                    <div class="bp-card-edit-overlay" (click)="openUploadPanel($event, p)">
                      <i class="pi pi-pencil" style="font-size:14px;color:#fff;"></i>
                    </div>
                  </div>
                </ng-template>
              ...
              <app-image-upload-panel *ngIf="uploadPanelProjectId === p.id"
                [projectId]="p.id" [existingCoverUrl]="p.cover_image_url || ''"
                [existingLogoUrl]="p.client_logo_url || ''" [existingCardColor]="p.card_color || ''"
                (imagesUpdated)="onImagesUpdated(p, $event)" (closed)="uploadPanelProjectId = ''">
        ...
        <div class="bp-body-right">
          <div class="bp-credits-card">
            ...
            <p class="bp-credits-desc">Each project fires simultaneously to all selected suppliers. Build and estimate for free — only spend a {{ creditLabel }} when ready to engage.</p>
          </div>
          ...
          <div *ngFor="let s of suppliers.slice(0,2)" class="bp-sup-card">
            ...
            <app-image-upload-panel *ngIf="uploadSupplierPanelId === s.id"
              [entityId]="s.id" type="supplier" [subtitle]="s.name"
              [existingCoverUrl]="s.hero_image_url || ''"
              (imagesUpdated)="onSupplierImagesUpdated(s, $event)" (closed)="uploadSupplierPanelId = ''">
            </app-image-upload-panel>
            <div class="bp-sup-body">
              <div class="bp-sup-name">{{ s.name }}</div>
              <div class="bp-sup-meta">{{ s.city || 'London' }}</div>
              <div class="bp-sup-footer">
                <div class="bp-sup-rating"><span class="bp-sup-star">&starf;</span>{{ s.rating || '4.8' }}<span class="bp-sup-count">({{ s.review_count || 24 }})</span></div>
                <div class="bp-sup-action">View &rarr;</div>
              </div>
            </div>
          </div>
          <p-button label="View all {{ supplierCount }} saved suppliers →" styleClass="w-full p-button-outlined mt-1" routerLink="/suppliers"></p-button>
        </div>
  ...
  styles: [`
    ...
    .bp-body { display:grid; grid-template-columns:384px minmax(400px,1fr) 384px; background:var(--color-bg); min-height:calc(100vh - var(--nav-height) - 120px - 80px - 64px); }
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  getCategoryClass(cat: string): string {
    const map: Record<string, string> = { 'set build': 'setbuild', 'av': 'av', 'audio visual': 'av', 'florals': 'florist', 'catering': 'catering' };
  ...
  loadSuppliers() {
    this.supplierService.getAll().subscribe({
      next: (suppliers: any[]) => { this.suppliers = suppliers || []; this.supplierCount = this.suppliers.length; this.cdr.detectChanges(); },
      error: () => {}
    });
  }
