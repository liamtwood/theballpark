...
/** pV2-04 — signal-backed page-settings store. One config per org_type
...
export class PageConfigService {
  ...
  /** The raw payload (null until loaded; empty object = server defaults). */
  readonly config = this._config.asReadonly();

  // Section flags — default ON when unset.
  readonly showStats = computed(() => sectionVisible(this._config(), 'showStats'));
  readonly showUpcoming = computed(() => sectionVisible(this._config(), 'showUpcoming'));
  readonly showQuickActions = computed(() => sectionVisible(this._config(), 'showQuickActions'));
  readonly showRecentActivity = computed(() => sectionVisible(this._config(), 'showRecentActivity'));
  readonly showCredits = computed(() => sectionVisible(this._config(), 'showCredits'));
  readonly showSavedSuppliers = computed(() => sectionVisible(this._config(), 'showSavedSuppliers'));

  // Hero + labels with defaults.
  ...
  readonly heroSubtitle = computed(() => this._config()?.heroSubtitle ?? '');
  readonly heroColor = computed(() => this._config()?.heroColor ?? CONFIG_DEFAULTS.heroColor);
  ...
  /** Typed accessor for the drawer's toggle rows. */
  sectionFlag(flag: SectionFlag): boolean {
    return sectionVisible(this._config(), flag);
  }
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      // Config is cosmetic-adjacent: defaults render fine without it, so boot
