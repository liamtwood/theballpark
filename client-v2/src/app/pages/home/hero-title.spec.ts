...
describe('heroTitle', () => {
  ...
  it('fixed mode uses the configured text', () => {
    expect(heroTitle('fixed', liam, 'Mission Control')).toBe('Mission Control');
  });

  it('fixed mode with empty text falls back to the greeting', () => {
    expect(heroTitle('fixed', liam, '   ')).toBe('Welcome back, Liam');
  });

  it('greeting for a signed-out edge (guards prevent it, function survives it)', () => {
